...
B-36 v1.2 `management_action_receipts` に実施済みReceiptが存在する場合、同一Evidence Ref集合に基づく同一種別の候補は再掲対象から除外する。
...
`evidence_fingerprint` はB-36 v1.2に従い、候補の全Evidence Refから `source_type / source_system / source_id` を抽出し、順序を正規化したうえで決定的に生成する。

`label` は表示文言でありfingerprintへ含めない。

単一Evidence Refだけを持つ候補では、結果的にその1件の `source_type + source_system + source_id` がfingerprintの根拠となる。
...
候補生成後、Decision Packへ載せる前に、候補のEvidence Ref集合からfingerprintを生成し、過去の `management_action_receipts` と照合する。

- 同一 `action_kind + evidence_fingerprint` で実施済み → 抑制
...
- 内容: 「早期相談と改善行動」
- 2件を正規化したfingerprintで `praise_delivered` Receiptを保存

翌日:
- 同じA+Bだけを根拠に再度「褒める」候補化 → 抑制

その後:
- 新しい根拠C `work_event / crm / event_789` が追加 → fingerprint変更 → 新しい称賛候補として表示可能
...
翌日:
- 同じ根拠集合・同じ `delegation_handled` → 抑制
...
- 実施済みの同一根拠集合・同一行動を翌日以降に繰り返し表示しない
